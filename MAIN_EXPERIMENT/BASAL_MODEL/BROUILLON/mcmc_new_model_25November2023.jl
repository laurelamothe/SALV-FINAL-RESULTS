...
#fitting procedure
@everywhere begin
    @model function fit_system(fixed_r, fixed_K, fixed_δ, data_ostreo_rep1, data_ostreo_rep2, data_virus_rep1, data_virus_rep2, time_ostreo, time_virus, prob_system, tmod, dt)
        ## Priors
        r ~ truncated(LogNormal(log(fixed_r), 0.05), 0.4, 2) = c
        µ ~ truncated(LogNormal(log(0.08), 1), 0, 1)
        K ~ truncated(LogNormal(log(fixed_K), 0.05), 1e5, 1e9) = c
        e ~ truncated(LogNormal(log(1e-10), 0.7), 1e-15, 1e-8) = c
        α ~ truncated(LogNormal(log(0.5), 1), 0, 1.4) # borne relatives à r
        ϵ ~ truncated(LogNormal(log(0.7), 1), 0, 1)
        η ~ truncated(LogNormal(log(2), 1), 0.01, 20)
        β ~ truncated(LogNormal(log(50), 0.5), 5, 100)
        δ ~ truncated(LogNormal(log(fixed_δ), 0.05), 1e-5, 0.5) = c
...
for i in 1:6#length(ostreo_list)
    ...
    modelfit = fit_system(fixed_r, fixed_K, fixed_δ, df_ostreo_rep1[i], df_ostreo_rep2[i], df_virus_rep1[i], df_virus_rep2[i], df_time_ostreo[i], df_time_virus[i], prob_system, tmod, dt)
    ...
    chain_df = DataFrame(
        ...
        β=chainarray[:, 8],
        δ=chainarray[:, 9],
        N0_rep1=chainarray[:, 10],
        N0_rep2=chainarray[:, 11],
        V0_rep1=chainarray[:, 12],
        V0_rep2=chainarray[:, 13],
        σ=chainarray[:, 14])
