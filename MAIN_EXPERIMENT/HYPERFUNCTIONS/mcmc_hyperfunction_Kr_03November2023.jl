...
@everywhere begin
    ...
    data1 = filter(row -> row.condition != "C_S2", data[1])
    data1.ϕ = exp.(data1.ϕ)
end
...
@everywhere begin
    ...
    @model function fit_hyperfunction_polynomial(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(-0, 0.2), -3, 3)
        b ~ truncated(Cauchy(0, 0.3), -2, 2)
        c ~ truncated(LogNormal(log(1), 0.3), 0.00001, 30)
    ...
    @model function fit_hyperfunction_normal(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(10), 0.8), 0.5, 1000)
...
begin
    ...
    p_reg = plot(layout=(2, 3), size=(1000, 600), margins=5mm, dpi=600, ylabel="ϵ", xlabel="Salinity (PSU)")
    ...
    for j in 1:length(list_model)
        for i in 1:length(letter_strain)
            ...
            elseif list_model_names[j] == "power"
                ...
                scatter!(p_reg, subplot=6, calib[7], calib[6] .* 1e7, markerstrokewidth=0, color=color_palette[i], label=name_strain[i], title="Power regression")
                samp = collect(5:0.2:40)
                a = calib[1].a
                b = calib[1].b
                c = calib[1].c
    ...
    mean_crit = DataFrame(
        ...
        AICc=[mean(filter(row -> row.model == "constant", AICc_list).AICc),
            ...
            mean(filter(row -> row.model == "power", AICc_list).AICc)],
        BIC=[mean(filter(row -> row.model == "constant", BIC_list).BIC),
