...
@everywhere begin
    ...
    @model function fit_hyperfunction_constant(data, salinity, sigma_mean, sigma_std)
        ## Priors
        c ~ truncated(LogNormal(log(1e-7), 1), 1e-13, 1e-4)
    ...
    @model function fit_hyperfunction_linear(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(1e-4, 0.001), 0, 0.05)
        b ~ truncated(LogNormal(log(1e-9), 0.3), 1e-13, 1e-7)
    ...
    @model function fit_hyperfunction_polynomial(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(-1e-15, 0.01), -1e-1, 0)
        b ~ truncated(Cauchy(1e-8, 0.3), -5, 10)
        c ~ truncated(LogNormal(log(1e-9), 0.3), 1e-13, 1e-4)
    ...
    @model function fit_hyperfunction_logistic(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(0.2), 0.8), 1e-13, 1)
        k ~ truncated(LogNormal(log(5e-8), 0.8), 1e-13, 0.05)
        s0 ~ truncated(LogNormal(log(10), 0.8), 1, 40)
    ...
    @model function fit_hyperfunction_normal(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(1e-5), 0.9), 1e-13, 1)
    ...
    @model function fit_hyperfunction_power(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(1e-7), 0.8), 1e-13, 1e-2)
        b ~ truncated(LogNormal(log(0.8), 0.8), 1e-13, 1)
        c ~ truncated(LogNormal(log(1e-9), 0.8), 1e-13, 1e-4)
...
begin
    ...
    p_reg = plot(layout=(2, 3), size=(1000, 600), margins=5mm, dpi=600, ylabel="ϕ (mL.day⁻¹)", xlabel="Salinity (PSU)")
