...
@everywhere begin
    ...
    @model function fit_hyperfunction_constant(data, salinity, sigma_mean, sigma_std)
        ## Priors
        c ~ truncated(LogNormal(log(20), 0.3), 0.0001, 30)

        σ ~ truncated(Normal(sigma_mean, sigma_std), 0, 1)
    ...
    @model function fit_hyperfunction_linear(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(1, 0.2), -1, 5)
        b ~ truncated(LogNormal(log(20), 0.3), 0.0001, 30)
    ...
    @model function fit_hyperfunction_polynomial(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(-1, 0.1), -5, 0)
        b ~ truncated(Cauchy(1, 0.3), -5, 10)
        c ~ truncated(LogNormal(log(20), 0.3), 0.0001, 30)
    ...
    @model function fit_hyperfunction_logistic(data, salinity, sigma_mean, sigma_std)
        ...
        k ~ truncated(LogNormal(log(15), 0.8), 5, 30)
        s0 ~ truncated(Cauchy(0, 0.3), -20, 10)
    ...
    @model function fit_hyperfunction_normal(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(100), 0.6), 1, 1000)
        #b ~ truncated(Cauchy(10, 0.3), 0, 30)
        σm ~ truncated(LogNormal(log(10), 0.3), 2, 30)
        µ ~ truncated(Cauchy(30, 0.3), 15, 40)
    ...
    @model function fit_hyperfunction_power(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(100), 0.8), 1, 1000)
        b ~ truncated(LogNormal(log(0.8), 0.8), 1e-13, 1)
        c ~ truncated(LogNormal(log(6), 0.8), 1, 20)
...
begin
    ...
    p_reg = plot(layout=(2, 3), size=(1000, 600), margins=5mm, dpi=600, ylabel="K (cell.mL⁻¹)", xlabel="Salinity (PSU)")
    ...
    for j in 1:length(list_model)
        for i in 1:length(letter_strain)
            ...
            if list_model_names[j] == "constant"
                ...
                scatter!(p_reg, subplot=1, calib[7], calib[6] .* 1e7, markerstrokewidth=0, color=color_palette[i], label=name_strain[i], title="Constant regression")
            ...
            elseif list_model_names[j] == "linear"
                ...
                scatter!(p_reg, subplot=2, calib[7], calib[6] .* 1e7, markerstrokewidth=0, color=color_palette[i], label=name_strain[i], title="Linear regression")
                samp = collect(5:0.2:40)
                a = calib[1].a
                b = calib[1].b
            ...
            elseif list_model_names[j] == "polynomial"
                ...
                scatter!(p_reg, subplot=3, calib[7], calib[6] .* 1e7, markerstrokewidth=0, color=color_palette[i], label=name_strain[i], title="Polynomial regression")
                samp = collect(5:0.2:40)
                a = calib[1].a
                b = calib[1].b
                c = calib[1].c
            ...
            elseif list_model_names[j] == "power"
                ...
                scatter!(p_reg, subplot=6, calib[7], calib[6] * 1e7, markerstrokewidth=0, color=color_palette[i], label=name_strain[i], title="Linear regression")
                samp = collect(5:0.2:40)
                a = calib[1].a
                b = calib[1].b
                c = calib[1].c
            ...
            elseif list_model_names[j] == "logistic"
                ...
                scatter!(p_reg, subplot=4, calib[7], calib[6] .* 1e7, markerstrokewidth=0, color=color_palette[i], label=name_strain[i], title="Logistic regression")
                samp = collect(5:0.2:40)
                a = calib[1].a
                k = calib[1].k
                s0 = calib[1].s0
                ...
                for t in 1:length(samp)
                    logi = logistic(a, k, s0, [samp[t]]) .* 1e7
                ...
                scatter!(p_reg, subplot=5, calib[7], calib[6] .* 1e7, markerstrokewidth=0, color=color_palette[i], label=name_strain[i], title="Normal regression")
                samp = collect(5:0.2:40)
                a = calib[1].a
                # b = calib[1].b
                σm = calib[1].σm
