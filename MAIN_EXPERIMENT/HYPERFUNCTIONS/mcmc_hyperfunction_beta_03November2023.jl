...
@everywhere begin
    ...
    @model function fit_hyperfunction_constant(data, salinity, sigma_mean, sigma_std)
        ## Priors
        c ~ truncated(LogNormal(log(200), 0.3), 1, 1000)
    ...
    @model function fit_hyperfunction_linear(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(0, 1), -30, 30)
        b ~ truncated(LogNormal(log(200), 0.3), 1, 1000)
    ...
    @model function fit_hyperfunction_polynomial(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(-30, 10), -100, 30)
        b ~ truncated(Cauchy(10, 7), -30, 800)
        c ~ truncated(LogNormal(log(200), 0.3), 1, 1000)
    ...
    @model function fit_hyperfunction_logistic(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(8), 0.8), 0.001, 1000)
        k ~ truncated(LogNormal(log(400), 0.2), 50, 1000)
    ...
    @model function fit_hyperfunction_normal(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(10000), 0.8), 10, 1000000)
    ...
    @model function fit_hyperfunction_power(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(1000), 0.8), 1, 10000)
        b ~ truncated(LogNormal(log(0.8), 0.8), 1e-13, 1)
        c ~ truncated(LogNormal(log(100), 0.8), 10, 1000)
...
begin
    ...
    p_reg = plot(layout=(2, 3), size=(1000, 600), margins=5mm, dpi=600, ylabel="β", xlabel="Salinity (PSU)")
