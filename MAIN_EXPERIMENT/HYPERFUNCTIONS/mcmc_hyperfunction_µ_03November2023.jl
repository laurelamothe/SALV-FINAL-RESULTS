...
@everywhere begin
    ...
    @model function fit_hyperfunction_constant(data, salinity, sigma_mean, sigma_std)
        ## Priors
        c ~ truncated(LogNormal(log(0.01), 0.3), 0.00001, 0.1)
    ...
    @model function fit_hyperfunction_linear(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(0, 0.2), -1, 1)
        b ~ truncated(LogNormal(log(0.01), 0.3), 0.00001, 0.1)
    ...
    @model function fit_hyperfunction_polynomial(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(Cauchy(0.001, 0.2), -0.1, 0.5)
    ...
    @model function fit_hyperfunction_logistic(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(0.5), 0.8), 0.001, 10)
        k ~ truncated(LogNormal(log(0.03), 0.2), 0.001, 1)
    ...
    @model function fit_hyperfunction_normal(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(0.1), 0.9), 0.0001, 10)
        σm ~ truncated(LogNormal(log(10), 0.3), 1, 30)
        µ ~ truncated(Cauchy(30, 0.3), 0, 40)
    ...
    @model function fit_hyperfunction_power(data, salinity, sigma_mean, sigma_std)
        ## Priors
        a ~ truncated(LogNormal(log(0.1), 0.8), 1e-6, 10)
        b ~ truncated(LogNormal(log(0.8), 0.8), 1e-13, 1)
        c ~ truncated(LogNormal(log(0.02), 0.8), 1e-4, 3)
...
function runMCMC(fitting_process, name_model, strain, param, data, nburn, nstep, nchain, list_hyper_param)
    data_mean = filter(row -> row.condition[1:2] == strain * "_", data1)[!, 6]
    salinities = filter(row -> row.condition[1:2] == strain * "_", data1).salinity
    sigma_mean = 0.000328909
    sigma_std = 0.000528683
...
begin
    ...
    p_reg = plot(layout=(2, 3), size=(1000, 600), margins=5mm, dpi=600, ylabel="µ", xlabel="Salinity (PSU)")
