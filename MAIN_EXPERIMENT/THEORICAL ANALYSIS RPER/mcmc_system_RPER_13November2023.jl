...
@everywhere begin
    ...
    @model function fit_system(fixed_r, fixed_K, fixed_δ, β_mean, η_mean, data_ostreo_rep1, data_ostreo_rep2, data_virus_rep1, data_virus_rep2, time_ostreo, time_virus, prob_system, tmod, dt)
        ...
        Rper ~ truncated(LogNormal(log(0.1), 0.7), 1e-20, 1)
...
for i in 13:13#length(ostreo_list)
    ...
    chain_df = DataFrame(
        ...
        β=chainarray[:, 6],
        Rper=chainarray[:, 7],
        N0_rep1=chainarray[:, 8],
        N0_rep2=chainarray[:, 9],
        V0_rep1=chainarray[:, 10],
        V0_rep2=chainarray[:, 11],
        σ=chainarray[:, 12])

    stats_chain = ess_rhat(chainfit)

    # Save Arrays as CSV
    CSV.write("./MAIN_EXPERIMENT/BASAL_MODEL/results/SALEXP_chains_system_final_basal_RPER_" * ostreo_list[i][1:4] * "_" * string(today()) * "_genotoul.csv", chain_df, writeheader=true)
    CSV.write("./MAIN_EXPERIMENT/BASAL_MODEL/results/SALEXP_statchains_system_final_basal_RPER_" * ostreo_list[i][1:4] * "_" * string(today()) * "_genotoul.csv", DataFrame(stats_chain), writeheader=true)
